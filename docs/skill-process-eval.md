...
- tasks are coherent fresh-agent assignments that retain coupled implementation context and embedded proof; independent, resource-compatible tasks fan out concurrently after boundaries are settled, dependent durable-output work uses later stages or an exceptional sequential baton pass, and no proof-only tasks are created;
