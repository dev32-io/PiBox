...
Managed E2E submits structured cases through `workflow_e2e_report`; the harness owns report JSON, canonical paths, and retention of supplied evidence. Fixers receive the complete exact current report plus any correction guidance. Do not ask E2E to author another terminal JSON, choose report filenames, or duplicate evidence references. Tool-argument errors are corrected within the evaluator attempt, not treated as product repair iterations. If reporting cannot finish, the evaluator remains interrupted and the workflow pauses; an explicit normal resume retries E2E, not the product fixer, without spending a product repair round. Normal reviewer reporting is unchanged.
