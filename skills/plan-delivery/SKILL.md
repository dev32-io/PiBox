...
5. **Route capability after decomposition** — Choose the smallest sufficient tier: `medium` for ordinary bounded implementation/review, `low` for bounded lookup, extraction, routine verification, or mechanical low-risk work. `local` requires current explicit user permission recorded in `rationale`. High/max require `tierJustification` explaining why medium is insufficient and further decomposition would damage the seam. High is the normal ceiling for hard work. Max is a very rare exception: additionally explain the specific reasoning bottleneck, why High is insufficient (observed or concretely expected), why better context/tools or safe decomposition cannot resolve it, and what Max should improve. Size, importance/security labels, urgency, uncertainty, one failed attempt, or the Nuke profile alone do not justify Max; when unsure about Max, choose High.
