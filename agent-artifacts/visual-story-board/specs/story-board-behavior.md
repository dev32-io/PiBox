...
Defines the user-visible behavior of the repository-local Visual Companion shell, Story Board catalog, nested story workspace, shared Markdown presentation, assisted E2E launcher, and compatibility with Architecture. V1 is a read-only view over persisted canonical artifacts and is intentionally not a live workflow dashboard.
...
- A standalone assisted E2E launcher binds a random loopback port, emits a machine-readable startup URL and bounded diagnostics, composes the real production backend/viewers/readers/routes/assets, and closes cleanly on explicit shutdown or process signals.
- The assisted launcher provides disposable canonical fixtures and fixture-only delayed-discovery and malformed-artifact recovery controls required by all approved browser journeys; it never substitutes a test-only renderer or requires a nested Pi/TUI session.
...
- Assisted E2E launcher is a standalone production-path loopback process that exposes the real extension backend and browser surface to a managed evaluator without requiring a nested Pi/TUI session.

## Actors

- Developer browsing the current repository
- Architecture visualization caller that opens a companion artifact directly
- Managed E2E evaluator exercising the browser through the assisted production-path launcher
...
- A managed E2E evaluator launches the assisted production-path fixture process, reads its machine-readable URL, drives the real browser surface, gathers bounded evidence, and stops the process without requiring another interactive Pi/TUI process.
...
- The assisted launcher receives a shutdown signal while discovery or a browser request is in flight.
- Fixture recovery or delay controls are requested outside the launcher’s disposable fixture boundary.
...
- A test-only Story Board renderer or nested Pi/TUI orchestration for browser E2E
