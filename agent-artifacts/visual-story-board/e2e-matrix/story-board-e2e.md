...
- Launch the production-path assisted Visual Companion fixture on a random loopback port against a disposable repository containing several healthy stories.
- Configure the fixture-only discovery seam to remain observably in flight long enough to exercise navigation without changing product rendering behavior.

#### Actions

- Open the machine-readable fixture URL in a browser.
...
- Bounded launcher diagnostic proving the hidden invariant that discovery began only after Story Board activation.

#### Safety

- Use only a disposable fixture repository and random loopback port.
- Terminate the assisted launcher and retain no browser credentials.
...
- Launch the assisted fixture against a disposable repository containing healthy stories plus malformed story index, task, document, and report fixtures.
...
- Replace the malformed disposable content with its valid counterpart using the fixture recovery control.
- Select Refresh.
...
- Modify only disposable fixture copies through the bounded fixture recovery control.
- Restore or remove the fixture and stop its launcher after the case.
...
- Launch the production-path assisted fixture with a valid architecture artifact and Story Board fixture.
...
- Bounded launcher diagnostic confirming one backend and deferred Story Board discovery.

#### Safety

- Use a disposable architecture artifact and loopback server.
- Remove the artifact and terminate the assisted launcher.
...
- Open the assisted fixture in a desktop browser and prepare a narrow mobile-sized viewport.
...
- Responsive and keyboard-accessible interaction through the production-path assisted fixture launcher

## Safety

- All browser journeys bind only to a random loopback port.
- The assisted fixture launcher composes the same production backend, registry, routes, readers, projector, Markdown assets, Architecture adapter, and Story Board UI as the extension.
- The evaluator starts the launcher, parses its machine-readable URL, uses bounded diagnostics only for named hidden invariants, and closes it after evidence collection.
- Every journey uses disposable canonical artifacts and cleans up its server and temporary files.
- Service controller lifecycle, parser unit behavior, cache semantics, path containment, sanitizer internals, and launcher cleanup are verified deterministically outside this user-journey matrix.
