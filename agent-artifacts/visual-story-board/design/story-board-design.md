# Visual Companion shell, Story Board, and assisted E2E design

## Design Goal

Evolve the architecture-specific companion into a fast reusable local application whose generic backend, viewer adapters, static artifact projection, browser presentation, assisted E2E seam, and design system remain independently understandable and testable.
...
- Implement Story Board as a viewer package with asynchronous discovery, branch-independent read-only canonical readers, a tolerant compatibility boundary, pure browser-safe projection, session-scoped single-flight caching, bounded routes, and dedicated browser assets.
...
- Provide a deterministic assisted fixture launcher that composes the same production backend and viewer registry as the extension, binds a random loopback port, prints machine-readable startup and diagnostics data, and shuts down cleanly. It exists as a verification and local-development seam, not an alternate product implementation.
- The launcher supplies disposable canonical fixtures plus fixture-only discovery-delay and malformed-artifact recovery controls, allowing a managed browser evaluator to exercise the extension without invoking the session-owned Pi tool or starting a nested Pi/TUI process.
...
- Unit checks cover status-to-column mapping, deterministic ordering, browser-safe projection, strict/fallback parsing, lazy reader boundaries, path containment, Markdown sanitization policy, cache single-flight behavior, invalidation, degraded sibling isolation, and launcher lifecycle.
...
- The assisted launcher is checked for random loopback binding, startup JSON, bounded diagnostics, exact production module composition, fixture coverage and integrity, no nested Pi dependency, signal cleanup, and absence of leaked resources.
- Fixture-driven browser journeys cover shell responsiveness during loading, nested Story Board navigation, documents, reports and evidence, malformed-artifact recovery through Refresh, Architecture compatibility, keyboard navigation, and narrow viewport behavior.
- The launcher must use the production backend, registry, routes, readers, projector, Markdown assets, and UI; verification fails if it silently substitutes a test-only data or rendering path.
- Visual evidence includes representative desktop and narrow-viewport screenshots using the approved neutral token system. Internal diagnostics are used only for hidden invariants such as deferred discovery and single-backend ownership.
...
- Story discovery and catalog loader: asynchronously enumerates agent-artifacts and reads the minimum index/intent data needed for story summaries without requiring each story's working branch to be checked out.
- Canonical readers: focused asynchronous read-only readers for work-item indexes, task manifests and narratives, story documents, evaluation manifests/reports/history/risk acceptance, and canonical evidence manifests/files.
...
- Assisted launcher API and CLI: accepts a repository root, optional architecture artifact, fixture-only discovery delay, loopback host, and random port; returns or emits URL/diagnostics/close controls and handles explicit and signal shutdown exactly once.
- Canonical fixture repository: healthy, complete, historical, malformed, report/evidence/image, delayed-discovery, and refresh-recovery states sufficient for all approved E2E cases.
...
- For E2E, the evaluator starts the assisted launcher, parses the machine-readable loopback URL, drives the production browser surface, queries only bounded fixture diagnostics for hidden invariants, records evidence, and terminates the launcher.
...
- The assisted launcher closes production resources exactly once on explicit close, SIGINT, or SIGTERM; fixture-only controls are unavailable outside its disposable configured root.

## Alternatives Considered

- A monolithic Node server containing service lifecycle, parsing, API projection, and browser behavior was rejected because it would couple unrelated viewer and workflow concerns.
...
- Embedding reports only inside task details was rejected because story-, stage-, whole-branch-, and E2E-scoped reports are independent artifacts.
- Requiring managed E2E to orchestrate a nested interactive Pi/TUI session was rejected in favor of the production-path assisted launcher plus deterministic lifecycle integration checks.
- A test-only fixture renderer was rejected because it could pass while the extension's real backend, routes, readers, or assets are broken.
