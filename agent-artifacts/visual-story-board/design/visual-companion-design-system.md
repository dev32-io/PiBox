...
- Use graphite-neutral canvas and surface colors: canvas #0c0e12; surfaces #12151b, #181c24, and #202631; hover surface #262d39; subtle, default, and strong borders #252b35, #313946, and #465163.
...
- Allow only restrained gradients: a subtle dark elevated-surface gradient, a near-neutral translucent header gradient, and a blue-to-muted-violet accent gradient reserved for rare emphasis.
...
- Use shadows only for true elevation: a restrained small surface shadow, an overlay shadow, and a left-cast drawer shadow.
...
- Cytoscape styling resolves semantic CSS variables through computed styles so graph concepts do not create a second palette.

## Verification Boundaries

- Static checks reject hard-coded shared palette values outside the token source and documented visualization data mappings.
- Computed-style checks prove shell, Story Board, Markdown, and Architecture consume the expected semantic tokens.
...
- Architecture semantic adapter that maps graph concepts to shared interaction and semantic tokens
- Visual fixtures and screenshots that make token drift observable across desktop and narrow viewports
...
- Architecture JavaScript reads resolved CSS variables when creating graph styles and refreshes those values when the viewer theme context changes.
...
- If the shared token asset cannot load, the shell remains readable through conservative CSS fallbacks.
- Unknown story or task statuses use neutral styling plus exact status text instead of guessing a semantic color.
- Long Markdown, labels, and task titles wrap without relying on fixed graph-oriented dimensions.
- Narrow viewports retain details through a sheet and do not hide the details surface.
- Token migration does not alter Architecture document data, graph semantics, or interaction behavior.

## Alternatives Considered

- Retaining the current Architecture palette and matching Story Board to it was rejected because its blue-green graph-specific values do not provide a professional general application system.
- Maintaining separate JavaScript and CSS token objects was rejected because duplicated values would drift.
...
- Runtime theme switching was deferred; the token architecture should permit later themes without expanding this story.
