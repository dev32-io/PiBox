...
export async function runInterruptedRecoveryScenario(): Promise<WorkflowScenarioResult> {
	...
	try {
		...
		const dead = await store.create({ repositoryId: "repo", workItemId: "recovery", taskId: "dead", role: "implementer", attempt: 1, state: "running", workspace: root, baseCommit: "base", pid: 2_147_000_000 });
		const handedOff = await store.create({ repositoryId: "repo", workItemId: "recovery", taskId: "handoff", role: "implementer", attempt: 1, state: "running", workspace: root, baseCommit: "base", pid: 2_147_000_001 });
		...
		if (recovered.some((run) => run.id === handedOff.record.id)) findings.push("Run with a durable handoff was incorrectly marked interrupted.");
		if ((await store.read(handedOff.record.id)).state !== "running") findings.push("Recovery mutated a handed-off run before reconciliation.");
...
export async function runDurableChangeRequestScenario(): Promise<WorkflowScenarioResult> {
	...
	try {
		...
		const { attempt } = await registry.startAttempt(reserved.id); await registry.markRunning(reserved.id, attempt.id, process.pid);
		...
		const fake = join(root, "fake-resume.mjs"); await writeFile(fake, "console.log(JSON.stringify({type:'message_end',message:{role:'assistant',content:[{type:'text',text:'resumed'}]}}));\n");
		const coordinator = new LaunchCoordinator(registry, "main:session", () => ({ command: process.execPath, args: [fake] }));
