...
test("usage contract accepts variable windows, renders remaining quota, and only accepts valid percentages", () => {
 assert.equal(normalizeWindows([{ percent: 0 }, { percent: 100 }, { percent: 101 }]).length, 2);
 assert.equal(formatUsageSnapshot({ provider: "x", observedAt: 0, windows: [{ usedPercent: 70 }] }), "30%");
});
