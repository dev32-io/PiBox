...
test("collaboration phases have focused boundaries and natural handoffs", async () => {
	...
	assert.match(delivery, /Tasks are fresh-agent boundaries; stages schedule those tasks/i);
	assert.match(delivery, /multi-task sequential stage is an exceptional agent baton pass/i);
	assert.match(delivery, /Every stage explicitly sets `mode` to `sequential` or `concurrent`/i);
	assert.match(delivery, /concurrent stage[\s\S]+per-task worktrees from one common base[\s\S]+one barrier/i);
	assert.match(delivery, /sequential stage[\s\S]+canonical working branch[\s\S]+prior task commits/i);
	assert.match(delivery, /stage is also a potential review boundary/i);
	...
	assert.match(delivery, /Plan fresh-agent boundaries/i);
	assert.match(delivery, /agent boundaries, not implementation-step boundaries/i);
	assert.match(delivery, /same discovery, reasoning, invariants, source context/i);
	assert.match(delivery, /independent and can run concurrently/i);
	assert.match(delivery, /durable output rather than the reasoning that produced it/i);
	assert.match(delivery, /oversized task at a coherent seam/i);
	assert.match(delivery, /never create proof-only, review-only, or verification-only tasks/i);
	assert.match(delivery, /Write complete tickets for one fresh agent/i);
	...
	assert.match(delivery, /After task boundaries are settled[\s\S]+every independent, resource-compatible task/i);
	...
	assert.match(delivery, /multi-task sequential stage only when each task independently warrants a fresh agent/i);
	assert.match(delivery, /concurrent stage[\s\S]+tasks cannot depend on each other/i);
	...
	assert.match(critic, /Prefer coherent assignments that fit one fresh-agent context/i);
});
