...
test("collaboration phases have focused boundaries and natural handoffs", async () => {
	...
	assert.match(run, /never edits source or task worktrees/i);
	assert.match(run, /installs dependencies/i);
	assert.match(run, /Git changes.*commits, merges, checkouts, resets, or conflict resolution/i);
	assert.match(run, /never launches replacement implementers, reviewers, or E2E agents/i);
	assert.match(run, /calls `evaluation_record` for runtime-owned reviews/i);
	assert.match(run, /accepts based on ad hoc checks/i);
	assert.match(run, /edits unrelated artifacts/i);
	assert.match(run, /uses `resume` to force a checkpoint or advance a run/i);
	assert.match(run, /checkpoint, fixer, or reviewer settlement fails[\s\S]+inspect the canonical recorded evidence once[\s\S]+one matching managed action[\s\S]+pause and surface the persistent failure/i);
