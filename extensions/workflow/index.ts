...
Keep clear, local, reversible work ad hoc. Use workflow-discover for material product discovery, diagnosis, alternatives, or optional durable understanding; use workflow-plan to make understood intent executable; use workflow-run for approved execution, evaluation, recovery, completion, and outcome briefing.

Canonical resources use work-item:<id> and work-item:<id>/<artifact|task|integration-unit|evaluation>:<id>. List before create and get before patch. Create a new parent once, then its children; patch matching drafts rather than duplicating them. Use workflow_apply_change only for coherent multi-resource decisions. Never edit agent-artifacts directly.

Initial approval is user-only through /workflow approve <work-item-id>. Routine approved amendments may retain approval within delegated intent; ask only when outcome, explicit constraints, consequential policy, privacy/security, irreversible effects, or a retained decision materially changes.
