...
- `Alt+Down` enters the grid. Arrow keys navigate it, Enter opens the selected overlay, and Up from the first row exits.
- Escape and unrelated keys are consumed while footer mode is active, preventing accidental editor changes or agent interruption.
- The shared overlay supports detail, setting, and action rows. Up from its first selectable row closes it; Escape is a no-op.
