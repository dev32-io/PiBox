...
test("eight logical E2E cases expand across the table and preserve disclosure, focus, and scroll on live refresh", async (t) => {
	...
	const cases = Array.from({ length: 8 }, (_, index) => ({ caseId: `E2E-${String(index + 1).padStart(3, "0")}`, title: `Journey ${index + 1}`, status: index === 7 ? "blocked" : "passed", executedActions: [`Action ${index + 1}`], observations: [index === 7 ? `<script>bad()</script> ${"Long observation ".repeat(80)}` : `Observed ${index + 1}`], ...(index === 7 ? { expected: "Expected <safe-result>", notes: "Notes <b>stay text</b>" } : {}), evidenceRefs: index === 7 ? [{ label: "evidence/proof.txt", memberPath: "evidence/proof.txt" }] : [], recorded: true }));
	...
	const report = { id: "final-e2e", title: "Final E2E", status: "testing", available: true, scope: { kind: "e2e" }, findingCount: 0, hasRiskAcceptance: false, findings: [], history: [], evidence: [], currentE2E: { phase: "testing", repairCount: 1, priorContext: false }, recordedE2E: { sourcePath: "evidence/report.json", sourceMemberPath: "evidence/report.json", result: "blocked", summary: "Recorded", findings: [{ summary: "Finding <img src=x>", severity: "major" }], cases, diagnostics: [] }, diagnostics: [] };
	...
	let detailRow = root.querySelectorAll<HTMLTableRowElement>("[data-e2e-case-detail-row]")[7]!; assert.equal(detailRow.hidden, false); assert.equal(detailRow.querySelector("td")?.colSpan, 4); assert.match(detailRow.textContent || "", /Long observation/); assert.match(detailRow.textContent || "", /Expected <safe-result>/); assert.match(detailRow.textContent || "", /Notes <b>stay text<\/b>/); assert.equal(detailRow.querySelector("script"), null); assert.equal(detailRow.querySelector("b"), null); assert.equal(detailRow.querySelectorAll("a").length, 1);
	assert.match(root.textContent || "", /Finding <img src=x>/); assert.equal(root.querySelector("img[src=x]"), null);
