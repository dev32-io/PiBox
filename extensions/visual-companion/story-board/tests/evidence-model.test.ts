...
test("evidence rejects symlinks, directories, absent manifests, and malformed contracts", async (t) => {
	...
	await put(root, "agent-artifacts/story/evidence/review/manifest.yaml", stringify({ schemaVersion: 1, evaluation: "review", entries: [{ path: "files/link.txt" }, { path: "files/directory" }] }));
	const metadata = await readEvidenceMetadata(root, "story", "review");
