...
import type { Diagnostic, DocumentDetail, DocumentGroup, DocumentSummary, Finding, ReportDetail, ReportSummary, StorySummary, StoryWorkspace, TaskCard, TaskDetail } from "./models.js";
...
async function regularFile(path: string, root: string): Promise<boolean> {
	const info = await lstat(path).catch(() => undefined);
	if (!info?.isFile() || info.isSymbolicLink()) return false;
	const [actual, actualRoot] = await Promise.all([realpath(path).catch(() => undefined), realpath(root).catch(() => undefined)]);
	return Boolean(actual && actualRoot && inside(actualRoot, actual));
}
...
export class StoryBoardReader {
	...
	private async readIndex(id: string): Promise<IndexRead> {
		const root = this.storyRoot(id)!;
		const display = `agent-artifacts/${id}/index.yaml`;
		let content: string;
		const indexPath = join(root, "index.yaml");
		if (!(await regularFile(indexPath, root))) return { value: {}, diagnostics: [diagnostic(display, "Story index is missing or not a contained regular file")] };
	...
	private async summary(id: string): Promise<StorySummary> {
		...
		if (safePath(intentPath)) {
			const candidate = join(this.storyRoot(id)!, intentPath);
			if (await regularFile(candidate, this.storyRoot(id)!)) intentExcerpt = await readFile(candidate, "utf8").then(excerpt, () => "");
		}
	...
	async readCatalog(): Promise<StorySummary[]> {
		...
		return orderStorySummaries(await Promise.all(ids.map((id) => this.summary(id))));
	}
	...
	private async readTaskCard(storyId: string, entry: { id: string; path: string; diagnostics: Diagnostic[] }, reportIds: string[]): Promise<TaskCard> {
		const root = this.storyRoot(storyId)!; const display = `agent-artifacts/${storyId}/${entry.path}`; const diagnostics = [...entry.diagnostics];
		let raw: RecordValue = {};
		try {
			const taskPath = join(root, entry.path);
			if (!(await regularFile(taskPath, root))) throw new Error("not regular");
	...
	private async readReportSummary(storyId: string, entry: { id: string; path: string; diagnostics: Diagnostic[] }): Promise<{ summary: ReportSummary; raw: RecordValue }> {
		const display = `agent-artifacts/${storyId}/${entry.path}`; const diagnostics = [...entry.diagnostics]; let raw: RecordValue = {};
		try {
			const evaluationPath = join(this.storyRoot(storyId)!, entry.path);
			if (!(await regularFile(evaluationPath, this.storyRoot(storyId)!))) throw new Error("not regular");
	...
	private async documents(storyId: string, raw: RecordValue): Promise<DocumentSummary[]> {
		...
		const checked = await Promise.all(projected.map(async (document) => {
			if (!document.path || await regularFile(join(this.repositoryRoot, document.path), this.storyRoot(storyId)!)) return document;
	...
	async readWorkspace(storyId: string): Promise<StoryWorkspace | undefined> {
		if (!this.storyRoot(storyId)) return undefined;
		const index = await this.readIndex(storyId); const story = await this.summary(storyId);
		const reportEntries = this.catalogEntries(storyId, index.value.evaluations, "evaluation");
		const reportReads = await Promise.all(reportEntries.map((entry) => this.readReportSummary(storyId, entry)));
		...
		const tasks = orderTaskCards(await Promise.all(taskEntries.map((entry) => this.readTaskCard(storyId, entry, reportTasks.get(entry.id) ?? []))));
		...
		const documents = await this.documents(storyId, index.value); const groups = new Map<DocumentGroup, DocumentSummary[]>(); for (const document of documents) groups.set(document.group, [...(groups.get(document.group) ?? []), document]);
	...
	async readTaskDetail(storyId: string, taskId: string): Promise<TaskDetail | undefined> {
		...
		const entryPath = `tasks/${taskId}/task.yaml`; const root = this.storyRoot(storyId)!; let raw: RecordValue = {};
		try { raw = record(parse(await readFile(join(root, entryPath), "utf8"))) ?? {}; } catch { /* card already carries diagnostic */ }
		...
		const readNarrative = async (name: string): Promise<string | undefined> => {
			...
			return await regularFile(path, root) ? readFile(path, "utf8").catch(() => undefined) : undefined;
		};
	...
	async readDocumentDetail(storyId: string, documentId: string): Promise<DocumentDetail | undefined> {
		...
		const path = join(this.repositoryRoot, summary.path); const root = this.storyRoot(storyId)!;
		if (!summary.path || !(await regularFile(path, root))) return { ...summary, available: false, diagnostics: [...summary.diagnostics, diagnostic(summary.path || `agent-artifacts/${storyId}/index.yaml`, "Document is missing or not a contained regular file")] };
	...
	async readReportDetail(storyId: string, reportId: string): Promise<ReportDetail | undefined> {
		if (!ID.test(reportId)) return undefined; const index = await this.readIndex(storyId); const entry = this.catalogEntries(storyId, index.value.evaluations, "evaluation").find((item) => item.id === reportId); if (!entry) return undefined;
		const { summary, raw } = await this.readReportSummary(storyId, entry); const evaluationRoot = join(this.storyRoot(storyId)!, "evaluations", reportId); const result = record(raw.result);
		...
		const reportPath = join(evaluationRoot, reportName); const body = await regularFile(reportPath, evaluationRoot) ? await readFile(reportPath, "utf8").catch(() => undefined) : undefined;
		...
		const riskAcceptance = riskPath && await regularFile(riskPath, evaluationRoot) ? await readFile(riskPath, "utf8").catch(() => undefined) : undefined;
		const attemptsRoot = join(evaluationRoot, "attempts"); const attemptsInfo = await lstat(attemptsRoot).catch(() => undefined);
		const attemptFiles = (attemptsInfo?.isDirectory() && !attemptsInfo.isSymbolicLink() ? await readdir(attemptsRoot).catch(() => []) : []).filter((name) => /^\d+-report\.md$/.test(name)).sort();
		const history = await Promise.all(attemptFiles.map(async (name) => { const attempt = Number.parseInt(name, 10); const attemptPath = join(attemptsRoot, name); const attemptBody = await regularFile(attemptPath, evaluationRoot) ? await readFile(attemptPath, "utf8").catch(() => undefined) : undefined; return { attempt, path: `agent-artifacts/${storyId}/evaluations/${reportId}/attempts/${name}`, ...(attemptBody ? { body: attemptBody } : {}), available: Boolean(attemptBody) }; }));
