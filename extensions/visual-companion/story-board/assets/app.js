...
export function createStoryBoardApp({ root, fetchImpl = fetch, navigationWindow = window.parent } = {}) {
  ...
  function e2eCaseDetailContent(report, item) {
    const refs = item.evidenceRefs?.length ? `<ul>${item.evidenceRefs.map((ref) => `<li>${ref.memberPath ? `<a href="${evidenceHref(report.id, ref.memberPath)}" target="_blank" rel="noopener noreferrer nofollow">${escapeHtml(ref.label)}</a>` : escapeHtml(ref.label)}</li>`).join("")}</ul>` : "<p>None recorded.</p>";
    const expected = item.expected == null ? "" : `<h4>Expected result</h4><p class="preserve-text">${escapeHtml(item.expected)}</p>`;
    const notes = item.notes == null ? "" : `<h4>Notes</h4><p class="preserve-text">${escapeHtml(item.notes)}</p>`;
    return `<div class="e2e-case-detail-content"><h4>Executed actions</h4>${textList(item.executedActions, "None recorded.")}${expected}<h4>Observations</h4>${textList(item.observations, "None recorded.")}${notes}<h4>Evidence references</h4>${refs}</div>`;
  }
  function e2eFindings(values = []) {
    return values.length ? `<ul>${values.map((finding) => typeof finding === "string" ? `<li>${escapeHtml(finding)}</li>` : `<li>${finding.severity ? `${badge(finding.severity)} ` : ""}${escapeHtml(finding.summary)}</li>`).join("")}</ul>` : "<p>No findings recorded in this evidence report.</p>";
  }
  function renderFinalE2E(report, storyId = state.route.storyId) {
    ...
    const source = recorded?.sourceMemberPath ? `<a href="api/evidence?story=${encodeURIComponent(storyId)}&evaluation=final-e2e&path=${encodeURIComponent(recorded.sourceMemberPath)}" target="_blank" rel="noopener noreferrer nofollow">${escapeHtml(recorded.sourcePath)}</a>` : escapeHtml(recorded?.sourcePath || "Unavailable");
    ...
    const recordedSection = recorded ? `<section class="recorded-e2e"><h3>Recorded E2E report</h3><dl class="metadata"><div><dt>Source</dt><dd>${source}</dd></div><div><dt>Evidence verdict</dt><dd>${badge(recorded.result)}</dd></div></dl><h4>Full summary</h4><p class="preserve-text">${escapeHtml(recorded.summary)}</p><h4>Findings</h4>${e2eFindings(recorded.findings)}${diagnostics(recorded.diagnostics)}${table}</section>` : `<section class="recorded-e2e"><h3>Recorded E2E report</h3><p>No case-results report is referenced by current E2E state.</p></section>`;
