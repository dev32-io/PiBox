...
export async function createAssistedFixtureRepository(): Promise<AssistedFixtureRepository> {
	...
	const stageBreakdown = Object.fromEntries(currentStages.map((stage, index) => {
		const categories = { implementation: 45_000 + index * 5_000, integration: 2_000, verification: 8_000, review: 12_000, e2e: 0 };
		return [stage.id, { workflowMs: Object.values(categories).reduce((sum, value) => sum + value, 0), categories, incompleteIntervals: 0, incompleteCategories: [] }];
	}));
	const categories = Object.values(stageBreakdown).reduce((totals, stage) => { for (const category of Object.keys(totals) as Array<keyof typeof totals>) totals[category] += stage.categories[category]; return totals; }, { implementation: 0, integration: 0, verification: 0, review: 0, e2e: 24_000 });
	await put(currentRoot, "state.yaml", stringify({ schemaVersion: 1, storyId: CURRENT_STORY_ID, status: "completed", activationOwner: { sessionId: "private-session", processInstanceId: "private-process", activationId: "private-activation" }, contracts: currentContracts, git: { canonicalBranch: "main", baseCommit: "base", integrationBranch: "private/integration", integrationWorktree: "/private/worktrees/current" }, stages: runtimeStages, finalReview: completedReview, e2e: { status: "completed", repairCount: 0, evidenceRefs: ["evidence/summary.txt", "evidence/nested/shot.png", "evidence/data.json", "evidence/archive.zip"], result: { code: "passed", summary: "E2E passed" } }, metrics: { workflowMs: Object.values(categories).reduce((sum, value) => sum + value, 0), categories, stageBreakdown, incompleteIntervals: 0, incompleteCategories: [] }, outcomeStatus: "written" }));
