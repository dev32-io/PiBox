...
The main session has four exclusive branch-local modes. Agent is the default direct-work mode with optional private session scratch. Orchestrator adds a hard plan/ledger/delegation protocol and mandatory scratch. Workflow alone authorizes workflow resource and execution tools, without weakening story review, planning, explicit start/resume, permission, Git, review, repair, or E2E gates. Designer adds `prompt/designer.md`, the closest lazily snapshotted `DESIGN.md`, and designer-handoff authority. Children receive none of the main-session mode controls.

Agent and Workflow keep a stable base prompt and skill catalog. Workflow tool schemas may be removed until first provider exposure; after exposure they remain resident but are mechanically blocked outside Workflow. Orchestrator and Designer intentionally change system authority, so transitions may cause a large prompt-cache miss while preserving logical conversation context. Restore mode from the active session-tree branch; new and legacy sessions default to Agent. Session scratch is opaque, private, best-effort `/tmp` state and never workflow or repository authority.
...
`/reload` is the only same-activation rebind through the process-global `SubagentService`; its runner reconnects automatically on the first explicit workflow demand, not during startup. Treat quit as crash; users should not quit while work is running. On that first demand, a later activation interrupts and fences old attempts, pauses the workflow, preserves Git/worktrees, and starts only fresh attempts after explicit resume. Ordinary startup performs no repository discovery or workflow disk restoration. Never add event replay, PID adoption, detached survival, file tails, or heartbeat recovery.
